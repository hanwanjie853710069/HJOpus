...
@property (nonatomic) int encoderBufferLength;
@end
...
- (NSData*) encodeData:(NSData *)data{
    ...
    unsigned long length = data.length - nPos;
    while(encoded + (kNumberOfSamplesPerChannel << 1) < length)
    {
        char returnValue = opus_encode(_encoder, shortdata, kNumberOfSamplesPerChannel, _encoderOutputBuffer, _encoderBufferLength);
        if (returnValue < 0) {
            return nil;
        }
